...
import { AmbientContext, useAmbientVars } from './hooks/useAmbientVars'
import { useMotionProfile } from './hooks/useMotionProfile'

function App() {
  const { ambient } = useMotionProfile()
  // Pointer and scroll, as --px / --py / --sy. Handed down through context and
  // written by each consumer onto its own root - never onto this node, where
  // an inherited custom property change would recalc style for the whole page.
  const ambientStyle = useAmbientVars()

  return (
    <AmbientContext.Provider value={ambientStyle}>
